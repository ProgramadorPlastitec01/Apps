        ...
        try {
            ...
            if (pageContext.getRequest().getAttribute("Usuario") != null) {
                ...
                if (pageContext.getRequest().getAttribute("Usuario").toString().equals("Registro")) {
                    ...
                    out.print("<h3><img id=\"Menu_registro\" src='Interfaz/Contenido/Iconos/Plus.png' width='20px' height='20px' alt='edit' title='Desplegar Menu' />Usuarios</h3>");
                    out.print("<script>");
                    out.print("$(Menu_registro).click(function() {");
                    out.print("$(\"#toggle\").toggle(\"slide\");");
                    out.print("});");
                    out.print("</script>");
                    out.print("<div style='display:none;border: 1px solid #016279;border-right: none;backgroung-color:#fff;position:absolute' id=\"toggle\">");
...
//                            + "val1.add(Validate.Confirmation, { match: 'Txt_password'} );</script>");
                    ...
                    out.print("});");
                    out.print("</script>");
                    out.print("<div style='display:block;border: 1px solid #016279;border-right: none;backgroung-color:#fff;position:absolute' id=\"toggle\">");
                    out.print("<div id='sidebar'>");
...
//                            + "val1.add(Validate.Confirmation, { match: 'Txt_password'} );</script>");
                    ...
                    out.print("<br /><br /><input type='submit' value='Actualizar' /><br />");
                    out.print("</form>");
                    out.print("<hr /><a href='Usuario?opc=7&Id_usuario=" + obj_usuario[0] + "' class='naranja'>Restablecer contraseña</a><br /><br /><br />");
                    out.print("<div class='cleaner'></div>");
                    out.print("</div> <!-- END of sidebar -->");
                    out.print("</div>");
                }
                if (filtro == null ? "" == null : filtro.equals("")) {
                    out.print("<div align='right' style='margin:0px;width:200px;float:right'><form action='Usuario?opc=1' method='post'><input type='text' name='fto' id='fto' placeholder='Buscar' onkeyup='javascript:this.value=this.value.toUpperCase();'/></form></div>");
                    lst_usuarios = jpacusa.Usuarios();
                } else {
                    lst_usuarios = jpacusa.Usuarios_filtro(filtro);
                    if (lst_usuarios == null) {
                        lst_usuarios = jpacusa.Usuarios();
                        out.print("<div align='right' style='margin:0px;width:200px;float:right'><form action='Usuario?opc=1' method='post'><b class='rojo'>El valor filtrado no obtubo resultados  </b><input type='text' name='fto' id='fto' placeholder='Buscar' value='" + filtro + "' onkeyup='javascript:this.value=this.value.toUpperCase();'/></form></div>");
                    } else {
                        lst_usuarios = jpacusa.Usuarios_filtro(filtro);
                        out.print("<div align='right' style='margin:0px;width:200px;float:right'><form action='Usuario?opc=1' method='post'><input type='text' name='fto' id='fto' placeholder='Buscar' value='" + filtro + "' onkeyup='javascript:this.value=this.value.toUpperCase();'/></form></div>");
                    }
                }
