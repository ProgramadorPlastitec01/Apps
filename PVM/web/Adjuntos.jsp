...
//                    String route = "\\\\172.16.2.117\\c\\files\\PVM\\adjuntos\\";
                        String route = "\\\\172.16.2.117\\h$\\Sistemas de informacion\\PVM\\adjuntos\\";
//                    String route = getServletContext().getRealPath("//Interfaz//Contenido//test//");
        ...
        result = actividad.RegistroVerificacion(hidden, file_name);
        int idI = Integer.parseInt(hidden.get(0).toString());
        int idTp = Integer.parseInt(hidden.get(4).toString());
        String filtro = hidden.get(5).toString();
        String tipoIF = hidden.get(6).toString();
        String dias = hidden.get(7).toString();
        request.getRequestDispatcher("Instrumento_medicion?opc=6&result=" + result + "&idI=" + idI + "&idTp=" + idTp + "&lstTipoIF=" + tipoIF + "&txt_dias=" + dias + "&txt_bus=" + filtro + "&fecha=").forward(request, response);
    }
%>