function Pager(tableName, itemsPerPage) {
    ...
    this.showPage = function(pageNumber) {
        ...
        newPageAnchor.style.color = '#5356ad';
    ...
    this.showPageNav = function(pagerName, positionId) {
        ...
        pagerHtml += '<span onclick="'+pagerName+'.last();" style="size:12px;" onmouseover="this.style.cursor=\'pointer\'"><b title="Ultimo" style="color:#000;">| Ultimo</b><b style="color:#000;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Total Paginas</b><b style="color:#5356ad;">|'+this.pages+'|</b></span>';
...
function Pager2(tableName, itemsPerPage) {
    ...
    this.showDivs = function(pageNumber){
        ...
        for( var i = 1 ; i <= this.divisores ; i++ ){
            if( divisorMostrar == i ){
                document.getElementById('P2dv'+i).style.display = '-webkit-inline-box';
            }else{
                document.getElementById('P2dv'+i).style.display = 'none';
            }
        }
    },
    ...
    this.showPage = function(pageNumber) {
        ...
        var oldPageAnchor = document.getElementById('P2pg'+this.currentPage);
        oldPageAnchor.className = 'P2pg-normal';
        oldPageAnchor.style.color = '#bbb';

        this.showDivs(pageNumber);
        this.currentPage = pageNumber;
        var newPageAnchor = document.getElementById('P2pg'+this.currentPage);
        newPageAnchor.className = 'P2pg-selected';
        newPageAnchor.style.color = '#5356ad';
    ...
    this.showPageNav = function(pagerName, positionId) {
        ...
        var pagerHtml = '<span onclick="' + pagerName + '.first();" style="size:12px;" onmouseover="this.style.cursor=\'pointer\'"><b title="Primero" style="color:#000;">Primero |</b></span>  ';
        pagerHtml += '<span onclick="' + pagerName + '.prev();" class="P2pg-normal" style="size:12px;font-family:Wingdings;" onmouseover="this.style.cursor=\'pointer\'"><b style="color:#000;" title="Anterior">&#8920;</b></span> | ';
        var divisor = 1;
        var idDivisor = 1;
        for (var page = 1; page <= this.pages; page++){
            if( idDivisor==1 ){
                pagerHtml += '<div id="P2dv'+divisor+'">';
            }
            idDivisor = idDivisor+1;
            pagerHtml += '-<span style="display: inline-table;font-weight: bold;color:#bbb" id="P2pg' + page + '" class="P2pg-normal" onclick="' + pagerName + '.showPage(' + page + ');"  style="size:12px;" onmouseover="this.style.cursor=\'pointer\'">' + page + '</span> ';
            if(idDivisor==this.cantidadDivs+1){
                idDivisor=1;
                divisor=divisor+1
                pagerHtml += '</div>';
        ...
        pagerHtml += ' | <span onclick="'+pagerName+'.next();" class="P2pg-normal" style="size:12px;font-family:Wingdings;" onmouseover="this.style.cursor=\'pointer\'"><b style="color:#000;" title="Siguiente">&#8921;</b></span> ';
        pagerHtml += '<span onclick="'+pagerName+'.last();" style="size:12px;" onmouseover="this.style.cursor=\'pointer\'"><b title="Ultimo" style="color:#000;">| Ultimo</b><b style="color:#000;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Total Paginas</b><b style="color:#5356ad;">|'+this.pages+'|</b></span>';
        element.innerHTML = pagerHtml;
    };
};
